...
export function addReplaceHandler(config: ReplaceHandler): void {
  if (!subscribeEvent(config)) return
  replace(config.type)
}
...
function xhrReplace(): void {
  ...
  replaceObj(originalXhrProto, 'open', (originalOpen: voidFun) => {
  ...
  replaceObj(originalXhrProto, 'send', (originalSend: voidFun) => {
    return function (this: any, ...args: any[]): void {
...
function fetchReplace(): void {
  ...
  replaceObj(_global, EVENTTYPES.FETCH, (originalFetch) => {
...
function historyReplace(): void {
  ...
  replaceObj(_global.history, 'pushState', historyReplaceFn)
  replaceObj(_global.history, 'replaceState', historyReplaceFn)
}
